from datetime import datetime # Исправили From на from
import os
import time
...
def parse_and_save_news():
    ...
    # Запускаем сбор и передаем результат в функцию сохранения
    all_current_news = collect_all_news()
    save_batch_to_xml(all_current_news)


# Главный блок, который запускает процесс по кругу
if __name__ == "__main__":
    print("Парсер запущен и работает в автоматическом режиме.")
   
    while True:
        try:
            # 1. Запускаем сбор новостей
            parse_and_save_news()
           
            # 2. Пишем в консоль, когда будет следующий сбор
            print("Жду 1 час до следующего сбора...")
           
            # 3. Замораживаем программу на 3600 секунд (1 час)
            time.sleep(3600)
           
        except Exception as e:
            # Если упал интернет или легла ссылка, скрипт не выключится,
            # а просто подождет 10 минут и попробует снова
            print(f"Произошла ошибка: {e}")
            print("Пробую повторить через 10 минут...")
            time.sleep(600)
